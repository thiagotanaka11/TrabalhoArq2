...
int sc_main(int argc, char *argv[])
{
    ...
    map<string,int> instruct_time{{"DADD",4},{"DADDI",4},{"ADDD", 4},{"DSUB",6},{"DSUBI",6},{"SUBS",6},
                                {"DMUL",10},{"MULD",10},{"DDIV",16},{"DIVS",16},{"MEM",2},{"MOVD",4}};
    top top1("top");
    botao.caption("START");
    ...
    sub->append("Tempos de latência", [&](menu::item_proxy &ip)
    {
        ...
        inputbox::text addd_t("ADDD", std::to_string(instruct_time["ADDD"]));
        inputbox::text subs_t("SUBS", std::to_string(instruct_time["SUBS"]));
        inputbox::text muld_t("MULD", std::to_string(instruct_time["MULD"]));
        inputbox::text divs_t("DIVS", std::to_string(instruct_time["DIVS"]));
        inputbox::text movd_t("MOVD", std::to_string(instruct_time["MOVD"]));
        if(ibox.show_modal(dadd_t,daddi_t,dsub_t,dsubi_t,dmul_t,ddiv_t,mem_t,addd_t,subs_t,muld_t,divs_t,movd_t))
        {
            ...
            instruct_time["ADDD"] = std::stoi(addd_t.value());
            instruct_time["SUBS"] = std::stoi(subs_t.value());
            instruct_time["MULD"] = std::stoi(muld_t.value());
            instruct_time["DIVS"] = std::stoi(divs_t.value());
            instruct_time["MOVD"] = std::stoi(movd_t.value());
        }
    });
    ...
    bench_sub->append("Teste do Trabalho",[&](menu::item_proxy &ip){
        string path = "in/benchmarks/teste_trab.txt";       
        inFile.open(path);
        if(!add_instructions(inFile,instruction_queue,instruct))
            show_message("Arquivo inválido","Não foi possível abrir o arquivo!");
        else
